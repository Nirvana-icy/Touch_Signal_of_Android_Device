package processing.test.touch_signal;
...
public class Touch_Signal extends PApplet {
...
public void draw()
{
  ...
  textSize(18);
  text("X:" + mouseX,6,6,0.3f*width,20);
  text("Y:" + mouseY,6,0.5f*height+6,0.3f*width,3*height/7+20);
  ...
  for(int i=1; i<width; i++) {
    ...
    stroke(255,0,0);
    ellipse(i, 0.3f*xvals[i],2,2);
    //Draw the point which indicator the Y value
    stroke(0,255,0);
    ellipse(i, 0.5f*height+yvals[i]/3,2,2);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  for (int i = 0; i < num; i++) {
    ...
    ellipse(mx[index], my[index], i + 16, i + 16);
  }
}
